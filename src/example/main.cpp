...
main(int, char**)
{
    ...
    memory_pool pool1(list_node_size, list_node_size * 10000);
    std::list<object, std_allocator<object, memory_pool>> list(pool1);


    constexpr std::size_t map_node_size = 16 + sizeof(std::pair<const int, object>);
    memory_pool pool2(map_node_size, map_node_size * 1000);
    ...
    memory_pool pool3(set_node_size, set_node_size * 1000);
    std::unordered_set<object, std::hash<object>, std::equal_to<object>,
            std_allocator<object, memory_pool>>
            set(pool3);
